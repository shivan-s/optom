...
class Refraction implements RefractionParams {
  ...
  specRx(vd: number): Refraction {
    const ocularRx = this.ocularRx();
    const sph = this.#effectivity(ocularRx.rawSph, vd);
    const cyl = this.#effectivity(ocularRx.rawSph + ocularRx.rawCyl, vd);
    return new Refraction({ sph, cyl, axis: this.axis, vd });
  }
}
